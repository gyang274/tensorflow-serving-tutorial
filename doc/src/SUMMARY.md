# Summary

## Overview

- [Docker Image of Tensorflow Serving Inception Models](README.md)

## Guide

- [Prerequisite](0x00.init.md)

- [Tensorflow Serving Setup](0x01.tfs.md)

- [Tensorflow Serving with Tensorflow Slim Models](0x02.slim.md)
  
  - [Tensorflow Serving with Slim Inception-V4](0x02b00.slim.inception.v4.md)
  
  - [Tensorflow Serving with Slim Inception-Resnet-V2](0x02b01.slim.inception.resnet.v2.md)
  
  - [A Unified Slim Client on PredictionService](0x02b02.slim.unified.client.md)

- [Add REST-API via uWSGI and Nginx](0x03.rest.md)

  - [uWSGI Application Setup](0x03b00.rest.uwsgi.md)
  
  - [Nginx Web Server Setup](0x03b01.rest.nginx.md)

- [Docker Images that Serves Tensorflow Slim Models](0x04.docker.md)

  - [Docker Image supports gPRC + protobuf](0x04b00.docker.grpc.md)

  - [Docker Image supports gPRC + protobuf, and REST + JSON](0x04b01.docker.rest.md)

## Miscellaneous

- [Miscellaneous](0xff.misc.md)

  - [Github](0xffb00.misc.github.md)
  
  - [Tensorflow](0xffb00.misc.tensorflow.md)
  
  - [uWSGI + Nginx](0xffb00.misc.uwsgi.nginx.md)
  
  - [Docker](0xffb00.misc.docker.md)
  
--

- [yg](https://github.com/gyang274/)

